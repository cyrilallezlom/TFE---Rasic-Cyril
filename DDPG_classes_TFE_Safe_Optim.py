# -*- coding: utf-8 -*-
"""
Created on Sat Apr 13 18:56:51 2024

@author: Cyril Rasic

Classes and usefull function for DDPG_Main
"""
...
class Environment(py_environment.PyEnvironment):
    ...
    def __init__(self, observations, non_observable_states, scaler, scalers, max_power: float, discount_rate: float,
                 nb_quarters_per_episode: int, col_MDP: int, EP: float, eta: float, bat_replacement_cost):
    ...
    def _step(self, action):
        ...
        #Battery life cycle cost : life degradation from Factoring "Cycle Aging Cost of Batteries
        #Participating in Electricity Markets" - Bolun Xu
        
        #cost if discharge
        if action < 0:
            cost = self._R * 5.24 * 10**(-4) * (abs(action)/4)**(2.03)
        else:
            cost = 0
        ...
        reward = float(delta_price) * (-network_charge_MW / 4) - cost
...
#%% -------Safety Layer with under constraint opti-------
def Safety_Layer(action, soc):    
    #constraints 
    C_1 = 1 
    C_2 = 0
    
    #battery E/P -> to do : EP as an argument
    EP_ratio = 1
    ...
    #lambda
    lambda_1 = (g_1 * action + soc - C_1)/(g_1**2)
    lambda_1 = 0 if lambda_1 <= 0 else lambda_1
    
    lambda_2 = (g_2 * action + (-soc) - C_2)/(g_2**2)
    ...
    if lambda_1 > lambda_2:
        ...
        i = 1
    #print(lambda_1, lambda_2, i)
    safe_action = action - lambda_list[i] * g_list[i] 
...
def collect_data(environment, policy, buffer, steps):
    ...
    for _ in range(steps):
        ...
        #SAFETY LAYER (Safe Exploration in Continuous Action Spaces - G. Dalal)
        #Correcting (the less possible) the action to avoid constraint violation on soc
        action = action_step.action[0].numpy()
        soc = state[0][-1].numpy()
        #print(soc)
        #print(action)
        
        safe_action = Safety_Layer(action, soc)
        safe_action_tensor = tf.convert_to_tensor([safe_action], dtype=tf.float64)
        
        # Create a new PolicyStep with the safe action
        safe_action_step = PolicyStep(
            action=safe_action_tensor,
            state=action_step.state,
            info=action_step.info
        )
        #print(soc,action,safe_action)
        #print(safe_action)
        next_time_step = environment.step(safe_action_step.action)
        traj = trajectory.from_transition(time_step, safe_action_step, next_time_step)
