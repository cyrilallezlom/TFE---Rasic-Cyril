...
class Environment(py_environment.PyEnvironment):
    ...
    def __init__(self, observations, non_observable_states, scaler, scalers, max_power: float, discount_rate: float,
        ...
        self._action_spec = array_spec.BoundedArraySpec(shape=(), dtype='float32', minimum=-1.0, maximum=1.0,
                                                        name="action")
        ...
        # a little bit
        self._observation_spec = array_spec.BoundedArraySpec(shape=(np.shape(observations)[1],), dtype='float32',
...
def Visualize(environment, policy, nbr_steps):
    ...
    for _ in range(nbr_steps):
        ...
        soc.append(time_step.observation[-1][-1])
